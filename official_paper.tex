...
\begin{itemize}
    ...
    \item Excitation parameters: wave type (e.g., plane wave via TFSF or antenna soft source), polarization, frequency $f$, wavelength $\lambda$ in free space and tissue, source amplitude $A$, and phase $\phi$ (for multi-antenna phase control)
\end{itemize}
...
In this study, the excitation is specified by: wave type (plane wave via TFSF or antenna soft source), frequency $f$ (e.g., 100 MHz within the 50--150 MHz band), polarization (e.g., linear along $\hat{x}$), and derived wavelength $\lambda$ in free space and in tissue to guide grid spacing. Source placement is defined in voxel coordinates for reproducibility, and phase offsets are used when evaluating multi-antenna focusing.
