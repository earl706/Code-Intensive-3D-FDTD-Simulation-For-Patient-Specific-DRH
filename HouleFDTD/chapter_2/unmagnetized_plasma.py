...
freq_in = np.array((50e6, 200e6, 3e16))
spread = 12 / freq_in[2]
pulse_delay = 40 / freq_in[2]
boundary_low = [0, 0]
boundary_high = [0, 0]
...
for time_step in range(1, nsteps + 1):
    ...
    t = time_step * dt
    pulse = np.exp(-0.5 * (((pulse_delay - time_step * dt) / spread) ** 2)) * np.sin(
        2 * np.pi * freq_in[2] * time_step * dt
    )
    dx[5] = pulse + dx[5]
    ...
    for k in range(1, ke):
        ...
        sx[k] = (
            (1 + np.exp(-vc[k] * dt)) * sxm1[k]
            - np.exp(-vc[k] * dt) * sxm2[k]
            + (omega**2 * dt / vc[k]) * (1 - np.exp(-vc[k] * dt)) * ex[k]
        )
...
def plot_e_field(
    ...
    plt.text(
        150,
        y_text_loc,
        "Time Domain, T = {}".format(timestep),
        horizontalalignment="center",
    )
    plt.plot(
        gb * gb_scaling_factor / gb[120], "k--", linewidth=0.75
...
def plot_amp(data, gb, freq, label, x_label):
    ...
    plt.text(
        150,
        1.2,
        "Freq. Domain at {} MHz".format(int(round(freq / 1e6))),
        horizontalalignment="center",
    )
    plt.plot(
        gb * 1 / gb[120], "k--", linewidth=0.75
    )  # The math on gb is just for scaling
