...
conductivity_correction[ia:ib, ja:jb][mask] = sigma * dt
...
for time_step in range(N_time_steps):
    ...
    Hx[:, :-1] -= (dt / mu0) * (Ez[:, 1:] - Ez[:, :-1]) / step_size
    Hy[:-1, :] += (dt / mu0) * (Ez[1:, :] - Ez[:-1, :]) / step_size
